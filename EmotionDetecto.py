...
grid_search = GridSearchCV(estimator=MLPClassifier(solver='adam', max_iter=1024), param_grid=param_grid, cv=3)
...
# Load the model
loaded_model = pickle.load(open('best_model_for_prediction.sav', 'rb'))

# Example usage of loaded model
example_file = "P:/archive/Actor_18/03-01-07-02-02-01-18.wav"
feature = extract_feature(example_file, mfcc=True, chroma=True, mel=True, contrast=True, tonnetz=True, zero_crossing_rate=True)
feature = scaler.transform(feature.reshape(1, -1))  # Scale the feature
